...
def resolve_batch(candidates: list[EntityCandidate]) -> list[Resolution]:
    """Resolve in sequence, so each decision can see the entities the previous ones
    created — resolving independently would create duplicates for the same person."""
    return [resolve(c) for c in candidates]


def find_potential_matches(
    candidate: EntityCandidate, threshold: float = 0.7
) -> list[Resolution]:
    """Candidates above `threshold` without committing to a merge. The AMBIGUOUS path
    already refuses to guess; this exposes the near-misses for review."""
    result = resolve(candidate)
    return [result] if result.score >= threshold else []

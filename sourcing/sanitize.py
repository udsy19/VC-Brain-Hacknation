...
# --- API surface re-exported by sourcing/__init__.py (branch B's public contract) ---
#
# The wrapping itself lives in core/llm.py, which is the choke point every model call
# goes through — Invariant #4 holds because it cannot be bypassed there. These are
# thin delegations so B's import surface keeps working without a second, divergent
# implementation of the same tags.
...
def wrap_content(content: str) -> str:
    """Mark text as data, never instructions."""
...
def strip_untrusted_wrapper(text: str) -> str:
    """Unwrap for display. Never call this before handing text to a model — the
    wrapper is the only thing telling it this content is not an instruction."""
    if not is_wrapped(text):
        return text
    start = text.index(UNTRUSTED_OPEN) + len(UNTRUSTED_OPEN)
    return text[start : text.index(UNTRUSTED_CLOSE)].strip()
